...
func runWorkspaceGC(ctx context.Context, rootDir string, args []string) error {
	if len(args) == 1 && isHelpArg(args[0]) {
		printGcHelp(os.Stdout)
		return nil
	}
	if len(args) > 0 {
		return fmt.Errorf("usage: gwst gc")
	}
	...
	renderer.Section("Info")
	renderer.Bullet(fmt.Sprintf("scanned: %d", result.Scanned))
	renderer.Bullet(fmt.Sprintf("candidates: %d", len(result.Candidates)))
	renderer.Bullet(fmt.Sprintf("skipped: %d", result.Skipped))

	if len(result.Candidates) == 0 {
		renderer.Blank()
		renderer.Section("Result")
		renderer.Bullet("no candidates to remove")
		return nil
	}

	renderer.Blank()
	renderer.Section("Candidates")
	for _, candidate := range result.Candidates {
		renderer.BulletWithDescription(candidate.WorkspaceID, candidate.Description, fmt.Sprintf("(repos: %d)", len(candidate.Repos)))
		var repoLines []string
		for _, repo := range candidate.Repos {
			name := formatRepoName(repo.Alias, repo.RepoKey)
			label := formatRepoLabel(name, repo.Branch)
			reason := strings.Join(repo.Reasons, ", ")
			if reason != "" {
				label = fmt.Sprintf("%s - %s", label, reason)
			}
			repoLines = append(repoLines, label)
		}
		renderTreeLines(renderer, repoLines, treeLineNormal)
	}

	renderer.Blank()
	confirm, err := ui.PromptConfirmInline(fmt.Sprintf("Delete %d workspaces?", len(result.Candidates)), theme, useColor)
	if err != nil {
		return err
	}
	if !confirm {
	...
	for i, candidate := range result.Candidates {
		output.Step(formatStepWithIndex("remove workspace", candidate.WorkspaceID, relPath(rootDir, workspace.WorkspaceDir(rootDir, candidate.WorkspaceID)), i+1, len(result.Candidates)))
		if err := workspace.Remove(ctx, rootDir, candidate.WorkspaceID); err != nil {
			failed = append(failed, fmt.Sprintf("%s: %v", candidate.WorkspaceID, err))
			continue
		}
		removed = append(removed, candidate.WorkspaceID)
	}

	renderer.Blank()
	renderer.Section("Result")
	for _, id := range removed {
		renderer.Bullet(fmt.Sprintf("%s removed", id))
	}
	if len(failed) > 0 {
		renderer.Bullet("failed to remove:")
		renderTreeLines(renderer, failed, treeLineError)
		return fmt.Errorf("failed to remove %d workspaces", len(failed))
	}
	return nil
}
