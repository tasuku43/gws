...
- When no `--repo` is provided:
  - If prompts are allowed, interactively select repos from the already fetched bare stores (same source as `gwst repo ls`). Candidates are bare-store repos only (no implicit fetch).
