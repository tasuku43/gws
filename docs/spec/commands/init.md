...
- Creates `manifest.yaml` if it does not exist, seeding it with an empty `workspaces` map.
...
- `manifest.yaml` exists and is a file (either newly created or previously present).

## Failure Modes
- Root directory not provided or cannot be accessed.
- Filesystem errors while creating directories or writing `templates.yaml`/`manifest.yaml`.
